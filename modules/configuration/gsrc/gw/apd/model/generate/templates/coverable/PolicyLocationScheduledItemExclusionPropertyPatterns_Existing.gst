...
%>
<%for (prop in schedExcl.ScheduleItemProperties.where(\f -> f.Type == APDFieldType.TC_LOCATION).sort(TemplateComparators.AttributeOrdering)) {
var currProp = pmExclusion.Schedules.ClauseSchedulePattern.ScheduledItemProperties.FKScheduledItemPropertyPattern?.firstWhere(\p -> p.CodeIdentifier == prop.CodeIdentifier)
if (currProp == null) {
