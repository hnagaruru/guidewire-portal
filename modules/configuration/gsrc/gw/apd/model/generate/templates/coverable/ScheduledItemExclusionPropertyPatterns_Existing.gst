...
%>
<%@ params(schedExcl : APDClause, type : APDFieldType, pmExclusion : ExclusionPattern) %>
<%
var newProperties = new ArrayList<APDTerm>()
var usedIndexes = new ArrayList<Integer>()
%>
<%for (prop in schedExcl.ScheduleItemProperties.where(\f -> f.Type == type).sort(TemplateComparators.AttributeOrdering)) {
var currProp = pmExclusion.Schedules.ClauseSchedulePattern.ScheduledItemProperties.ScheduledItemPropertyPattern?.firstWhere(\p -> p.CodeIdentifier == prop.CodeIdentifier)
if (currProp == null) {
