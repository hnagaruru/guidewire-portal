...
%>
<%for (prop in schedCond.ScheduleItemProperties.where(\f -> f.Type == APDFieldType.TC_TYPEKEY).sort(TemplateComparators.AttributeOrdering)) {
var currProp = pmCondition.Schedules.ClauseSchedulePattern.ScheduledItemProperties.TypekeyScheduledItemPropertyPattern?.firstWhere(\p -> p.CodeIdentifier == prop.CodeIdentifier)
if (currProp == null) {
