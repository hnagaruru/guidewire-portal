...
@Export
class ${coverable.QualifiedName}ScheduleCondImplBase extends AbstractProductModelScheduleImpl<${coverable.QualifiedName}ScheduleCond> {
  ...
  override function createAndAddScheduledItem() : ScheduledItem {
    var scheduledItem = new ${coverable.QualifiedName}ScheduleCondItem(Owner.Branch)
  ...
  override function removeScheduledItem(item : ScheduledItem) {
    Owner.removeFrom${coverable.QualifiedName}ScheduledItems(item as ${coverable.QualifiedName}ScheduleCondItem)
  ...
  override property get CurrentAndFutureScheduledItems() : KeyableBean[] {
    ...
    Owner.Branch.OOSSlices
        ...
        .each(\p -> {
          var matchingSlicedScheduleCond = p.${APDGosuReferencesHelper.getReferenceFromPeriodToAllCoverablesOfThisType(coverable)}.ConditionsFromCoverable.firstWhere(\c -> c.FixedId == Owner.FixedId) as ${coverable.QualifiedName}ScheduleCond
    ...
    return schedItems.map(\item -> item as ${coverable.QualifiedName}ScheduleCondItem).toTypedArray()
  }

  override property get ScheduleNumberPropInfo() : IPropertyInfo {
    return ${coverable.QualifiedName}ScheduleCondItem.Type.TypeInfo.getProperty("ScheduleNumber")
  }
