...
%>
<%for (prop in schedCov.ScheduleItemProperties.where(\f -> f.Type == APDFieldType.TC_TYPEKEY).sort(TemplateComparators.AttributeOrdering)) {
var currProp = pmCoverage.Schedules.ClauseSchedulePattern.ScheduledItemProperties.TypekeyScheduledItemPropertyPattern?.firstWhere(\p -> p.CodeIdentifier == prop.CodeIdentifier)
if (currProp == null) {
