...
uses gw.web.rules.APDRulesHelper
...
class SideBySideCovTermInfo {
  ...
  property get Editable() : boolean {
    if (CovInfo.Selected and CovTerm != null) {
      var editableByRules = APDRulesHelper.isEditable(_coverage.OwningCoverable, CovTermPattern)
      if (!editableByRules) {
        return false
      }
      if (CovTerm typeis PackageCovTerm) {
  ...
  property get Visible() : boolean {
    var isVisible = true
    if (CovTerm != null) {
      isVisible = APDRulesHelper.isVisible(CovInfo.Coverable, CovTermPattern)
    } else if (CovInfo.Coverable != null) {
      isVisible = APDRulesHelper.isVisible(CovInfo.Coverable, CovInfo.CoveragePattern)
    }
    return isVisible
  }
