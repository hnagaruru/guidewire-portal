...
uses gw.apd.model.export.xsd.apdproductimport.anonymous.elements.Rule_DefaultCalculatedValue_Arguments
uses gw.apd.model.export.xsd.apdproductimport.anonymous.elements.Rule_DefaultCalculatedValue_Arguments_Argument
...
class APDProductExportHelper {
  ...
  static var _logger : Logger = APDLoggerCategory.EXPORT
  ...
  private function buildImportProductLine(productLine: APDProductLine) :Import_APDProductLine {
    return new Import_APDProductLine(){
      ...
      :Edition                = buildProductLineEdition(productLine.Editions),
  ...
  private function buildProductLineEdition(editions : APDProductLineEdition[]) : List<Import_APDProductLine_Edition> {
    var xml = new ArrayList<Import_APDProductLine_Edition>()
    editions.each(\edition -> {
      xml.add(new Import_APDProductLine_Edition() {
        :EditionCode    = edition.EditionCode,
        :EffectiveDate  = edition.EffectiveDate.XmlDateTime
      })
    })
    return xml
  }
  ...
  private function buildClauses(clauses: APDClause[]): Coverable_Clauses{
    ...
    clauses.whereTypeIs(APDCoverage).sortBy(\e -> e.Sequence).each(\coverage -> ret.addChild(
        new Coverable_Clauses_APDCoverage() {
          ...
          :ParentClause = coverage.ParentClause != null ? new () {:PublicId = coverage.ParentClause.PublicID} : null,
    ...
    clauses.whereTypeIs(APDCondition).each(\condition -> ret.addChild(
        new Coverable_Clauses_APDCondition(){
          ...
          :ParentClause = condition.ParentClause != null ? new () {:PublicId = condition.ParentClause.PublicID} : null,
    ...
    clauses.whereTypeIs(APDExclusion).each(\exclusion -> ret.addChild(
        new Coverable_Clauses_APDExclusion(){
          ...
          :ParentClause = exclusion.ParentClause != null ? new () {:PublicId = exclusion.ParentClause.PublicID} : null,
  ...
  private function buildTerms(coverage :APDClause): Clause_Terms {
    ...
    coverage.Terms.sortBy(\e -> e.Sequence).each(\ term ->
        ret.addChild(new Clause_Terms_APDTerm(){
          ...
          :GenerateAsClauseTerm = term.ScheduleItemAttribute?term.GenerateAsClauseTerm:null,
          :Type        = term.Type.Code,
  ...
  private function buildAttributeRules(attribute : APDAttribute) : Attribute_Rules {
    ...
    attribute.Rules.each(\rule -> {
      if (rule typeis APDAttributeRule && !(rule typeis APDDropdownEntryRule)) {
          ...
          case APDRuleType.TC_MAX :
            switch (attribute.Type) {
              ...
              case APDFieldType.TC_TYPEKEY :
                ...
                if (rule.DefaultCalculatedValue != null and attribute.DropDownType == APDDropDownType.TC_OPTION) {
                  attRule.DefaultCalculatedValue = buildDefaultCalculatedValue(rule)
                }
                break
              ...
              case APDFieldType.TC_BIGDECIMAL :
                ...
                if (rule.DefaultCalculatedValue != null) attRule.DefaultCalculatedValue = buildDefaultCalculatedValue(rule)
                break
        ...
        attRule.PublicId = rule.PublicID
        attRule.Edition = rule.Edition.EditionCode
    ...
    attribute.Rules.each(\rule -> {
      if (rule typeis APDDropdownEntryRule) {
        var attRule = new Attribute_Rules_APDDropDownEntryRule(){
          ...
          :Edition          = rule.Edition.EditionCode,
  ...
  private function buildDefaultCalculatedValue(rule : APDAttributeRule) : Rule_DefaultCalculatedValue {
    final var defaultCalculatedValue = new Rule_DefaultCalculatedValue() {
      :Function = rule.DefaultCalculatedValue.Func.Code,
      :Arguments = buildArguments(rule)
    }
    return defaultCalculatedValue
  }

  private function buildArguments(rule : APDAttributeRule) : Rule_DefaultCalculatedValue_Arguments {
    final var defaultCalculatedValueArguments = new Rule_DefaultCalculatedValue_Arguments()
    rule.DefaultCalculatedValue.FunctionArguments.each(\ref -> {
      var defaultCalculatedValueArgument = new Rule_DefaultCalculatedValue_Arguments_Argument() {
        :PublicId = ref.Attribute.PublicID
      }
      defaultCalculatedValueArguments.addChild(defaultCalculatedValueArgument)
    })
    return defaultCalculatedValueArguments
  }
          ...
          if (rule typeis APDAttributeRule) {
            switch (rule.Attribute.Type) {
              ...
              case APDFieldType.TC_INTEGER :
                ...
                break
              case APDFieldType.TC_VARCHAR :
                if (elm.DefaultStringValue != null) ruleElem.DefaultStringValue = elm.DefaultStringValue
                break
            }
          }
          break
  ...
  private function buildRuleConditions(elm : APDRuleElement) : Rule_RuleElements_APDRuleElement_RuleConditions {
    ...
    elm.RuleConditions.each(\rc -> {
      var condition = new Rule_RuleElements_APDRuleElement_RuleConditions_APDRuleCondition(){
        ...
        :Attribute        = (rc.Attribute == null) ? null : new() {:PublicId = rc.Attribute.PublicID},
        :Clause           = (rc.Clause == null) ? null : new() {:PublicId = rc.Clause.PublicID},
