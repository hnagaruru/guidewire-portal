...
%>
<%@ params(terms : APDTerm[], pmExclusion : ExclusionPattern) %>
<%
...
var currTerm = pmExclusion.CovTerms.GenericCovTermPattern.firstWhere(\t -> t.CodeIdentifier == term.CodeIdentifier)
if (currTerm == null) {
    newBools.add(term)
} else {
...
var currTerm = pmExclusion.CovTerms.DirectCovTermPattern.firstWhere(\t -> t.CodeIdentifier == term.CodeIdentifier)
if (currTerm == null) {
    newNums.add(term)
} else {
...
var currTypekeyTerm = pmExclusion.CovTerms.TypekeyCovTermPattern.firstWhere(\t -> t.CodeIdentifier == term.CodeIdentifier)
var currOptionTerm = pmExclusion.CovTerms.OptionCovTermPattern.firstWhere(\t -> t.CodeIdentifier == term.CodeIdentifier)
var currPackageTerm = pmExclusion.CovTerms.PackageCovTermPattern.firstWhere(\t -> t.CodeIdentifier == term.CodeIdentifier)
