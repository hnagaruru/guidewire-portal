...
%>
<%@ params(schedCov : APDClause, pmCoverage : CoveragePattern) %>
<%
var newProperties = new ArrayList<APDTerm>()
var usedIndexes = new ArrayList<Integer>()
%>
<%for (prop in schedCov.ScheduleItemProperties.where(\f -> f.Type == APDFieldType.TC_LOCATION).sort(TemplateComparators.AttributeOrdering)) {
var currProp = pmCoverage.Schedules.ClauseSchedulePattern.ScheduledItemProperties.FKScheduledItemPropertyPattern?.firstWhere(\p -> p.CodeIdentifier == prop.CodeIdentifier)
if (currProp == null) {
  ...
  usedIndexes.add(colNum)%>
      ${PolicyLocationScheduledItemPropertyPattern.renderToString(prop, colNum)}
<%}%>
<%}%>
...
for (prop in newProperties) {
  ...
  usedIndexes.add(nextIndex)%>
      ${PolicyLocationScheduledItemPropertyPattern.renderToString(prop, nextIndex)}
<%}%>
